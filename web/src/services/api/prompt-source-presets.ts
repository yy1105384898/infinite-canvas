...
const cangyuanPromptLibraryScript = `// 沧元提示词库：通过本站反向代理分页读取，保留原始标签和分类。
const pageSize = 100;
const items = [];
let page = 1;
let total = Infinity;
while (items.length < total) {
  const data = await fetchJson(\`/api/prompts?page=\${page}&pageSize=\${pageSize}\`);
  const rows = Array.isArray(data.items) ? data.items : [];
  if (!rows.length) break;
  total = Number(data.total) || items.length + rows.length;
  for (const item of rows) {
    ...
    items.push(makePrompt({
      id: String(item.id || \`cangyuan-\${leftPad(items.length + 1)}\`),
      title,
      prompt,
...
export const DEFAULT_PROMPT_SOURCES: PromptSource[] = [
    { id: "cangyuan-prompt-library", name: "沧元提示词库", githubUrl: "https://canvas.cangyuansuanli.cn/prompts", enabled: true, script: cangyuanPromptLibraryScript },
