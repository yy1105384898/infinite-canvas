...
export async function fetchPrompts({ keyword = "", tag = [], category = ALL_PROMPTS_OPTION, modality = ALL_PROMPTS_OPTION, page = 1, pageSize = 20 }: { keyword?: string; tag?: string[]; category?: string; modality?: string; page?: number; pageSize?: number } = {}) {
    const items = await getPrompts();
    const normalizedKeyword = keyword.trim().toLowerCase();
    const normalizedPage = Math.max(1, page);
    const normalizedPageSize = Math.max(1, Math.min(100, pageSize));
    const withoutTagFilter = filterPrompts(items, { keyword: normalizedKeyword, category, modality, tags: [] });
    const filtered = filterPrompts(items, { keyword: normalizedKeyword, category, modality, tags: tag });
...
function filterPrompts(items: Prompt[], options: { keyword: string; category: string; modality: string; tags: string[] }) {
    return items.filter((item) => {
        ...
        if (isActiveOption(options.modality) && item.modality !== options.modality) return false;
...
function defaultPrompt(id: string, title: string, prompt: string, coverUrl: string, tags: string[], preview: string): Omit<Prompt, "category" | "githubUrl"> {
    return { id, title, coverUrl, modality: "image", prompt, tags, preview, createdAt: "", updatedAt: "" };
}
