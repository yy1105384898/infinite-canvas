    ...
    persist(
        ...
        }),
        {
            ...
            merge: (persisted, current) => {
                ...
                const savedSources = Array.isArray(persistedState.sources) && persistedState.sources.length ? persistedState.sources.map((item) => createPromptSource(item)) : DEFAULT_PROMPT_SOURCES;
                const sources = DEFAULT_PROMPT_SOURCES.filter((source) => source.id === "cangyuan-prompt-library" && !savedSources.some((item) => item.id === source.id)).concat(savedSources);
                return {
                    ...current,
                    sources,
